...
- "Основные области ответственности" обычно структурирована в два уровня:
  крупные пронумерованные блоки (например "1. Строительство и капитальный
  ремонт скважин", "2. Маркшейдерские и геодезические работы") и под-пункты
  внутри них (1.1, 1.2, 1.3…). Названия КРУПНЫХ БЛОКОВ (без под-пунктов) →
  key_results (это конечные области результата, не процессы). Сами под-пункты
  (конкретные действия) → responsibilities. Если такого двухуровневого деления
  нет — весь список идёт в responsibilities, а key_results оставь [].
...
- "Полномочия" / "принимает решение самостоятельно" → decides_alone;
  ...
  "рекомендует" / "выносит на рассмотрение" → recommends. Не выводи это из
  глаголов в обязанностях («организует», «контролирует») — это разные вещи:
  обязанность описывает действие, а не право окончательного решения. Если
  отдельного раздела про полномочия/DoA нет — оставь decides_alone/
  requires_approval/recommends пустыми, а не угадывай по обязанностям.
- "Навыки, знания и опыт" (требуемое образование/стаж/квалификация) и
  "Дополнительная информация о должности" (членство в комиссиях и т.п.) →
  organizational_context (объедини через двойной перевод строки, если есть оба).
  Это контекст для оценки Know-How, не выдумывай его из обязанностей.
    ...
    *,
    ...
    dossier = JobDossier(
        ...
        import_metadata=ImportMetadata(
            ...
            # 2000 символов обрезали реальные документы до раздела с требованиями
            # к знаниям/опыту (Know-How) — этот текст должен дойти до агента
            # оценки (build_user_message сериализует весь JobDossier, включая
            # это поле), а не потеряться молча на середине документа.
            raw_text_preview=raw_text[:_RAW_TEXT_PREVIEW_LIMIT],
        ),
    )
